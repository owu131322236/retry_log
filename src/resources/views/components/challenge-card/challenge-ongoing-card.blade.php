@props(['challenge'])
<div class="bg-white rounded-2xl shadow-lg min-w-[350px] p-6 hover:scale-105 transition-all">
    <div class="flex justify-between items-center mb-4">
        ...
        <div class="relative">
            ...
            <ul class="options-menu hidden bg-gray-100 absolute top-10 right-0 p-2 w-fit rounded-lg shadow-lg z-10
                ...
                <li>
                    <form action="{{ route('challenges.restart',$challenge) }}">
                        @csrf
                        @method('POST')
                        <button class="replicate-btn hover:text-gray-500 min-w-[80px] my-2">コピー</button>
                    </form>
                </li>
                <li>
    ...
    <div class="flex items-center text-sm text-gray-500 space-x-4 mb-6">
        <div class="flex items-center">
            <span>現在: {{$challenge->current_steak}}日</span>
        </div>
        <div class="flex items-center">
            <span>最高: {{ $challenge->max_steak }}日</span>
        </div>
        <div class="flex items-center">
            <span>開始: {{$challenge->start_date}}</span>
        </div>
    </div>
    <button id="addHabit" class="flex justify-center w-full bg-gray-900 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-600 transition duration-300">
        今日の習慣を記録
    </button>
    <!-- 使用先に書く用のJS -->
    <!-- <script>
