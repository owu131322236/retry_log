...
class ProductionDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        collect($challenges)->each(function ($c) use ($randomDate) {
            $randomNum = rand(1, 5);
            ...
            $state = $endDate->isFuture()
                ? fake()->randomElement([ChallengeState::NOT_STARTED, ChallengeState::IN_PROGRESS])
                : fake()->randomElement([ChallengeState::FAILED, ChallengeState::INTERRUPTED, ChallengeState::COMPLETED]);
            ...
            ChallengeLog::factory()
                ->count(rand(0, 20))
                ->state(function () use ($startDate, $endDate) {
                    return [
        ...
        $targetType = fake()->randomElement([Post::class, Comment::class]);
