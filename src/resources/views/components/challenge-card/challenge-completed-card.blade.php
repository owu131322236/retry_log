...
<div class="bg-white rounded-2xl shadow-lg min-w-[350px] h-fit p-6 hover:scale-105 transition-all">
    ...
    @endif
        <div class="flex space-x-3 items-center ">
            ...
            <h3 class="text-xl font-bold text-gray-900">{{$endedchallenge->title}}</h3>
        </div>
        @if($endedchallenge->state=='completed')
        <span class="bg-blue-200 text-blue-800 text-sm font-medium px-3 py-1 rounded-full">達成済み</span>
        @else
        <span class="bg-red-200 text-red-800 text-sm font-medium px-3 py-1 rounded-full">中断</span>
        @endif
    </div>
    ...
    <div class="flex items-center text-sm text-gray-500 space-x-4 mb-6 opacity-60">
        <div class="flex items-center">
            ...
            <span>現在: {{ $endedchallenge->current_steak }}日</span>
        </div>
        <div class="flex items-center">

            <span>最高: {{ $endedchallenge->max_steak }}日</span>
        </div>
        @if ($endedchallenge->stare=='completed')
        <div class="flex items-center">
            <span>達成: {{$endedchallenge->end_date }}</span>
        </div>
        @else
        <div class="flex items-center">
            <span>中断: {{ $endedchallenge->end_date  }}</span>
        </div>
        @endif
